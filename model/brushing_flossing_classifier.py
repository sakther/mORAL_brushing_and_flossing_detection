...
brushing_model_file_name_all_feature = 'model\AB_model_brushing_all_features.model'
...
flossing_model_file_name_all_feature = 'model\AB_model_flossing_all_features.model'
...
def classify_brushing(X: pd.DataFrame):
    ...
    clf = get_model(brushing_model_file_name_all_feature)
    X=X.values
    X = X[:,4:]
    preds = clf.predict(X)
...
def classify_flossing(X: pd.DataFrame):
    ...
    clf = get_model(flossing_model_file_name_all_feature)
    X=X.values
    X = X[:,4:]
    preds = clf.predict(X)
